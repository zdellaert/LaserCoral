...
- "Pocillopora_acuta_HIv2___RNAseq.g8520.t1"
- "Pocillopora_acuta_HIv2___RNAseq.g23688.t1"
- "Pocillopora_acuta_HIv2___RNAseq.g3131.t1" 
...
```{r}
# to avoid any confusion, remove any chromosomes frome the genome that are not represented in our 3606 DE genes
genome_filtered <- genome[genome@ranges@NAMES %in% seqlevels(promoters_500_UP)]

names(genome_filtered) == seqlevels(promoters_500_UP)

# extract the positive strand transcripts, where start < end
#pos_promoters <- promoters_500_UP_df %>% filter(strand == "+")

# extract the negative strand transcripts, where start > end
#neg_promoters <- promoters_500_UP_df %>% filter(strand == "-")
...
for (chromosome in unique(seqnames(promoters_500_UP))) {
  ...
  promoters_chromosome <- list()
  
  for (i in 1:nrow(promoters_500_UP_filtered)) {
    promoter <- subseq(genome_filtered, start= promoters_500_UP_filtered$start[i], end= promoters_500_UP_filtered$end[i])
    names(promoter) <- promoters_500_UP_filtered$ID[i]
    
    promoters_chromosome[[i]] <-  promoter
  }
  
  promoters_all[[chromosome]] <- promoters_chromosome
}
