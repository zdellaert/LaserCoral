...
for (chromosome in unique(seqnames(promoters_500_UP))) {
  ...
  for (i in 1:nrow(promoters_500_UP_filtered)) {
    ...
    promoters_chromosome[[promoters_500_UP_filtered$ID[i]]] <-  promoter
  }
  
  #collapse this big list into a "DNAStringSetList" object, which takes less memory
  collapsed <- DNAStringSetList(unlist(promoters_chromosome))
  
  #append this chromosome list to the DNAStringSetList for all chromosomes
  promoters_all <- c(promoters_all, unlist(collapsed))
}
