...
##### Cluster 11: Sensory
cluster_11 <- Oral_enriched_clustered %>% dplyr::filter(GO.cluster == 11) %>% pull(GO.ID)
cluster_11_genes <- unique(unlist(genes_by_GO[cluster_11]))
cluster_11_genes_DE <- DE_05_SwissProt %>% filter(query %in% cluster_11_genes)
write.csv(cluster_11_genes,"../output_RNA/differential_expression/semantic-enrichment/cluster_gene_lists/Oral_enrich_BP_Sensory.csv",row.names=FALSE)
...
cluster_5_9_16 <- Aboral_enriched_clustered %>% dplyr::filter(GO.cluster == 5|
                                                               GO.cluster == 9|
                                                               GO.cluster == 10|
                                                             GO.cluster == 11|
                                                           GO.cluster == 12|
...
showSigOfNodes(BP_Aboral, score(elim_Aboral)[names(score(elim_Aboral)) %in% cluster_5_9_16],  firstSigNodes = length(cluster_5_9_16), useInfo = 'all')

cluster_5_9_16_genes <- unique(unlist(genes_by_GO[cluster_5_9_16]))
cluster_5_9_16_genes_DE <- DE_05_SwissProt %>% filter(query %in%cluster_5_9_16_genes)
write.csv(cluster_5_9_16_genes,"../output_RNA/differential_expression/semantic-enrichment/cluster_gene_lists/Aboral_enrich_BP_development.csv",row.names=FALSE)
