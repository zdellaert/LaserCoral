...
cluster_8_9 <- unique(c(genes_by_GO[["GO:0050955"]], 
               genes_by_GO[["GO:0050906"]], 
               genes_by_GO[["GO:0007600"]], 
               genes_by_GO[["GO:0007605"]],
               genes_by_GO[["GO:0050982"]]))
...
ggsave("../output_RNA/differential_expression/semantic-enrichment/BP_Dend_Clusters_Oral.png", final, width = 5, height = 7, dpi = 300)
```
...
ggsave("../output_RNA/differential_expression/semantic-enrichment/BP_Dend_Clusters_Aboral.png", final, width = 7, height = 6.5, dpi = 300)
```
                        ...
                        mutate(is_top = GO.ID == top_sig_GO,
                               pvalue = Oral_elim.pvalue,
                                term_wrapped = str_wrap(`term`, width = 25),
                                 cluster_label = factor(Cluster.name, levels = unique(Cluster.name))) %>%
...
p_oral <-ggplot(plotting_oral, 
  ...
  geom_point(alpha = 0.7, color = "#FD8D3C") +
    facet_grid(str_wrap(cluster_label, width = 20) ~ ., 
               scales = "free_y", space = "free_y") +
    geom_text_repel(data = plotting_oral %>% filter(is_top),
